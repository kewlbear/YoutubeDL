...
private func isType(_ object: PythonObject,
  ...
  return pyObject != PyObjectPointer(&_Py_FalseStruct)
}

extension Bool : PythonConvertible, ConvertibleFromPython {
  public init?(_ pythonObject: PythonObject) {
    guard isType(pythonObject, type: PyObjectPointer(&PyBool_Type)) else { return nil }

    let pyObject = pythonObject.ownedPyObject
    defer { Py_DecRef(pyObject) }

    self = pyObject == PyObjectPointer(&_Py_TrueStruct)
  }
...
extension Range : ConvertibleFromPython where Bound : ConvertibleFromPython {
  public init?(_ pythonObject: PythonObject) {
    guard isType(pythonObject, type: PyObjectPointer(&PySlice_Type)) else { return nil }
...
let _NonePointer = Python.None.reference.borrowedPyObject
